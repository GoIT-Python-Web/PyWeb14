...
from lesson01.app_refresh.auth import create_access_token, create_refresh_token, get_email_form_refresh_token, \
    get_current_user, Hash
from lesson01.app_refresh.db import User, get_db
...
async def signup(body: UserModel, db: Session = Depends(get_db)):
    exist_user = db.query(User).filter_by(email=body.username).first()
    if exist_user:
...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    db.commit()
    pprint({"access_token": access_token, "refresh_token": refresh_token, "token_type": "bearer"})
